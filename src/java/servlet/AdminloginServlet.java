...
public class AdminloginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Admin loginedAdmin = null;
        boolean hasError = false;
        String errorString = null;
 
        if (password == null ||  password.length() == 0 || staffid == null || staffid.length() == 0) {
            hasError = true;
            errorString = "Required staffid, password";
        } else {
            Connection conn = MyUtils.getStoredConnection(request);
            try {
                // Find the user in the DB
               loginedAdmin = DBUtils_admin.findAdmin(conn, staffid, password);
        ...
        if (hasError) {
                ...
                loginedAdmin = new Admin();
                loginedAdmin.setStaffid(staffid);
                loginedAdmin.setPassword(password);
        ...
        // And redirect to userInfo page.
            ...
            MyUtils.storeLoginedAdmin(session, loginedAdmin);
